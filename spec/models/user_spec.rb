...
describe User do
  ...
  it "生年月日（年）を選択して下さい" do
    user = build(:user, year_id:"")
    user.valid?
    expect(user.errors[:year_id]).to include("選択して下さい")
  end

  it "生年月日（月）を選択して下さい" do
    user = build(:user, month_id:"")
    user.valid?
    expect(user.errors[:month_id]).to include("選択して下さい")
    end
  it "生年月日（日）を選択して下さい" do
    user = build(:user, date_id:"")
    user.valid?
    expect(user.errors[:date_id]).to include("選択して下さい")
    end
  it "電話番号を入力してください" do
